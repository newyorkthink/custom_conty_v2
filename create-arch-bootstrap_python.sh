...
:<< 'NOTE'
note: Forbidden is fine, but note the download package , sometime have loss some packages
anaconda 不要放到aur_pkgs ，copy时间很久

sudo cp -r ./sharedfolder/Downloads/fonts/* ./custom_conty/root.x86_64/usr/share/fonts
sudo cp -r ./sharedfolder/Downloads/applications/* ./custom_conty/root.x86_64/usr/share/applications
...
python_dev_tools="python python-pip python-setuptools python-wheel python-virtualenv python-virtualenvwrapper python-requests \
	ipython cython python-pipenv python-poetry python-pytest python-pylint python-pylint-venv python-coverage python-pyqt5 python-pyqt6"

qt_dependencies='qt5ct qt5-base qt5-wayland qt5-tools qt5-declarative qt5-multimedia qt5-webengine kvantum-qt5 \
	qt6ct qt6-base qt6-wayland qt6-tools qt6-declarative qt6-multimedia qt5-webengine kvantum'

development_base_tools="base-devel devtools binutils meson autoconf automake strace patch git cmake gcc-libs"

xorg_packages="xorg-server xorg-server-common xorg-server-devel xorg-xinit xorg-xinput xorg-xwayland xorg-server-xephyr xorg-xrandr xorg-xset xorg-xprop xorg-font-util xorg-docs"

# Packages to install
# You can add packages that you want and remove packages that you don't need
# Apart from packages from the official Arch repos, you can also specify
# packages from the Chaotic-AUR repo
export packagelist="${fonts_management_tools} ${common_fonts} ${python_dev_tools} ${qt_dependencies} ${development_base_tools} ${xorg_packages} \
	yay chromium ibus gtk3 zsh bash shellcheck xfce4-terminal thunar file-roller qview curl wget nwg-drawer anaconda pycharm-community-edition spyder jupyter-notebook jupyter-server nodejs"

# If you want to install AUR packages, specify them in this variable
export aur_packagelist="miniconda3 firefox-esr-bin rstudio-desktop-bin postman-bin"
