...
:<<'NOTE'
最新说明，该文件就不要删除，就是
umi ocr 是没办法封装到conty, 首先就是archlinux 源是tar 压缩包，无法在安装的时候解压，
其次，用拷贝的方式也是不行的，会出现报错：
OSError: [Errno 30] Read-only file system: '/opt/Umi-OCR_Linux_Paddle_2.1.5/UmiOCR-data/logs'
...
sudo cp -r ./sharedfolder/Downloads/Umi-OCR_Linux_Paddle_2.1.5/ ./custom_conty/root.x86_64/opt/
NOTE
...
export packagelist="git ibus gdk-pixbuf2 gdk-pixbuf-xlib libpulse libxcomposite libxcrypt-compat libxrandr postgresql-libs openssl-1.1 pyside6 python-attrs python-brotli python-cairo \
	python-cffi python-cryptography python-defusedxml python-docutils python-fonttools python-jinja python-lxml python-lz4 python-mako python-opengl python-pillow python-pip \
	python-psutil python-pymupdf python-pyopenssl python-pyqt5 python-pyqt6 python-reportlab python-requests python-setuptools python-simplejson python-tornado python-typing_extensions \
	python-wheel python-wxpython qt5-tools qt6-charts qt6-multimedia qt6-quick3d qt6-virtualkeyboard qt6-wayland qt6-webengine unixodbc gendesk which autoconf automake bison \
	debugedit diffutils flex gc groff guile m4 make patch pkgconf base-devel yay sudo texinfo python-pyproject-hooks python-build python-evdev python-installer python-xlib"

# If you want to install AUR packages, specify them in this variable
export aur_packagelist="python-pynput"
