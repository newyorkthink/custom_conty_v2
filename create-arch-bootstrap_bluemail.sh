...
:<<'NOTE'
bluemail 不能封装appimage，封装后的appimage 是无法显示邮件，其次，封装成conty也是，无法调用浏览器
...
sudo cp -r ./sharedfolder/Downloads/bluemail/* ./custom_conty/root.x86_64/
NOTE
...
export packagelist="git base-devel ibus gdk-pixbuf2 libappindicator-gtk3 libnotify libxss libxtst nss imagemagick squashfs-tools alsa-lib at-spi2-core bash cairo dbus ffmpeg fontconfig \
	freetype2 gcc-libs glib2 glibc gtk3 hicolor-icon-theme libpulse libx11 libxcb libxcomposite libxdamage libxext libxfixes libxrandr libxss libxt nspr pango libnotify firefox"
